...
class CorefTransformerLayer(torch.nn.Module):
    ...
    def forward(self, inputs, coref_context, mask):
        ...
        if coref_context is None:
            ...
            self.log_attention(coref_context.src_text, coref_context.context_docs, coref_context.chain_id,
                               gate_vals, top_attn_mt, top_attn_ctx, coref_context.chain_map)

        out = gated_context + inputs
        return self.feed_forward(out)

    def log_attention(self, src_text, context_docs, chain_id, gate_vals, top_attn_mt, top_attn_ctx, chain_map):
        torch.save((src_text, context_docs, chain_id, gate_vals, top_attn_mt, top_attn_ctx, chain_map),
